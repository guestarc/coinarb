...
class MoneyMetalsAdapter(DealerAdapter):
    ...
    def parse_buy_text(cls, text, sku):
        ...
        pat = re.escape(product) + r'.*?(?:1\s*-\s*9|1\+)\s*\|?\s*\$([0-9,]+\.\d{2})'
        ...
        return Observation(cls.dealer_id, sku, 'ask', float(m.group(1).replace(',', '')), cls.BUY_URL, product,
                           quantity_min=1, inventory_status='available')
    ...
    def collect_with_evidence(self, canonical_sku):
        buy_text, buy_evidence = self.fetch_text_with_browser_fallback(self.BUY_URL)
        sell_text, sell_evidence = self.fetch_text_with_browser_fallback(self.SELL_URL)
        observations = []
        errors = []
        try:
